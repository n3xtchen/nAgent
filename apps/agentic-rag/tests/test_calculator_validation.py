...
async def run_validation():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            ...
            print(f"  准确性 (Correctness): {c_res.value:.1f}/5.0 - {c_res.reason}")
            ...
            print(f"  相关性 (Answer Relevance): {r_res.value:.1f}/5.0 - {r_res.reason}")
            metrics_summary["answer_relevance"].append(r_res.value)

            # 3. 推理步数
            s_res = await reasoning_steps_metric.ascore(trace=trace)
            print(f"  推理步数 (Reasoning Steps): {s_res.value:.0f} - {s_res.reason}")
            ...
            if retrieved_docs:
                ...
                print(f"  忠实性 (Faithfulness): {f_res.value:.1f}/5.0 - {f_res.reason}")
            ...
            results.append({
                ...
                "faithfulness": f_res.value if retrieved_docs else None,
            })

        except Exception as e:
            print(f"❌ 测试 {test_case['id']} 执行失败: {e}")
            import traceback
            traceback.print_exc()
